...
def key_exchange():
	...
	out_dir = NET_PATH + 'S' + '/IN'
	msgs = sorted(os.listdir(out_dir))
	if len(msgs) - 1 > last_read['S']:
		next_msg = msgs[last_read['S'] + 1]
		sender = next_msg[0]
		receiver = next_msg[-1]
		with open(out_dir + '/' + next_msg, 'rb') as f: msg = f.read()
		in_dir = NET_PATH + 'S' + '/OUT'
		with open(in_dir + '/' + receiver, 'wb') as f: f.write(msg)
		last_read['S'] += 1
...
if CLEAN:
	...
	in_dir = NET_PATH + 'S' + '/IN'
	for f in os.listdir(in_dir): os.remove(in_dir + '/' + f)
	out_dir = NET_PATH + 'S' + '/OUT'
	for f in os.listdir(out_dir): os.remove(out_dir + '/' + f)
